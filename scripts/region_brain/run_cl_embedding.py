...
from preprocessing.load_dataset import RegionEmbBatchedDataset, SequentialBatchIterator
...
def main(config):
    ...
    for batch_file in batch_files:
        dataset = RegionEmbBatchedDataset(metadata, batch_files=batch_file)
        ...
        for batch in progress_bar:
            embs, guids, struct_names = batch["emb"], batch["guid"], batch["struct_name"]
            with torch.no_grad():
                proj_embs = model(embs)
            all_proj_embs.append(proj_embs.cpu())
            all_guids.extend(guids)  # list of strings
            all_struct_names.extend(struct_names) 
    ...
    df_embs.insert(0, "LabelName", all_struct_names)
