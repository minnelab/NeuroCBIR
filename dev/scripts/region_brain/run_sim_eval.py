...
def main(config):
    ...
    embedding_cols = [col for col in df_embs.columns if col not in ["GUID", "LabelName"]]
    dataset["features"] = dataset[embedding_cols].apply(lambda row: row.to_numpy(), axis=1)
    logging.info(f"Dataset size after merging and filtering: {len(dataset)} entries.")
    logging.info(f"Columns in dataset: {dataset.columns.tolist()}")
    
    # Determine structures to evaluate
    ...
    for struct_name in struct_names:
        ...
        # Select subset of dataset for current structure
        subset = dataset[dataset["LabelName"] == struct_name].reset_index(drop=True)
        logging.debug(f"Subset size for structure {struct_name}: {len(subset)} entries.")
        logging.debug(f"Subset columns:\n{subset.columns}")
        
        # Filter dataset and msssim_df for current structure
        msssim_df_struct = msssim_df[msssim_df["struct_name"] == struct_name].reset_index(drop=True)
        queries_guid_list = msssim_df_struct["query_guid"].unique().tolist()
        subset_queries = subset[subset["GUID"].isin(queries_guid_list)].reset_index(drop=True)
        logging.info(f"Number of queries per project: {subset_queries['project'].value_counts().to_dict()}")
        logging.info(f"Number of queries per partition: {subset_queries['partition'].value_counts().to_dict()}")
        
        # Compute retrieval
        logging.info("Computing retrieved cases...")
        retrieval_df = retrieve_topk_for_queries(subset, subset_queries, top_k=len(subset))
        ...
        all_metrics[struct_name]["standard"]["spearman_r"] = summarize_spearman_results(spearman_df)
        
        # Bias metrics per specified column
        all_metrics[struct_name]["bias"]["spearman_r"] = {}
        for col in config["bias_columns"]:
            logging.info(f"  → Evaluating bias by column: '{col}'...")
            grouped = spearman_df.groupby(col)
            all_metrics[struct_name]["bias"]["spearman_r"][col] = {}
            for group_name, group_df in grouped:
                all_metrics[struct_name]["bias"]["spearman_r"][col][group_name] = summarize_spearman_results(group_df)
    ...
    if logging.getLogger().level == logging.DEBUG:
        ...
        plt.savefig(os.path.join("./tmp/figures_reg", "spearman_correlation_histogram.png"))
        ...
        for _, row in tqdm(merged.groupby("query_guid")):
            ...
            plt.savefig(os.path.join("./tmp/figures_reg", f"spearman_scatter_{row['query_guid'].iloc[0]}.png"))
            plt.close()
